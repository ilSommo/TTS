...
def __process_transcript(file_path: str):
    ...
    for fragment in index[1:]:
        ...
        txt = fragment[2]
        audio['text'] = normalizer_call(txt).lower()
...
def __process_data(dataset_path, val_size, test_size, seed_for_ds_split):
    ...
    save(dataset_path / "train_manifest_stt_raw.json", train_entries)
    save(dataset_path / "val_manifest_stt_raw.json", validate_entries)
    save(dataset_path / "test_manifest_stt_raw.json", test_entries)
